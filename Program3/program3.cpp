...
int main()
{
    ...
    rmqt::Result<rmqa::Consumer> consRes = vhost->createConsumer(
        ...
        [&producer, &id](rmqp::MessageGuard& guard)
        {
            ...
            if (s == "connect")
            {
                ...
                std::cout.flush();
                rmqt::Message queueId(
                        bsl::make_shared<bsl::vector<uint8_t>>(id.cbegin(), id.cend()));
                auto connStatus = producer->send
                        (
                            queueId,
                            "return", // This routes to program1-queue
                            [](const rmqt::Message& message,
                               const bsl::string& routingKey,
                               const rmqt::ConfirmResponse& response)
                            {});
                if (connStatus != rmqp::Producer::SENDING)
                {
